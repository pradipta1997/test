...
class Menus extends CI_Model
{
    ...
    public function fetch_parent_navi()
    {
        if ($this->session->userdata('id_group') == 1) {
            $query = $this->db->query("SELECT * FROM tbl_user_menu WHERE parent_id = 0 AND show_in_menu = 1 AND is_active = 1 ORDER BY sort_order ASC");
        }

        if ($this->session->userdata('id_group') != 1) {
            $sgroup = $this->session->userdata('user_login')['id_sgroup'];
            $query = $this->db->query("SELECT * FROM tbl_user_menu AS UM , tbl_user_permission AS UP WHERE UM.id_menu = UP.id_menu AND UP.per_select = 1 AND UP.id_sgroup = $sgroup AND UM.parent_id =0 AND UM.is_active = 1 AND UM.show_in_menu = 1 ORDER BY UM.sort_order ASC");
        }
    ...
    public function fetch_child_navi()
    {
        ...
        if ($this->session->userdata('id_group') != 0) {
            ...
            $query = $this->db->query("SELECT * FROM tbl_user_menu AS UM , tbl_user_permission UP WHERE UM.id_menu = UP.id_menu AND UP.per_select = 1 AND UP.id_sgroup = $sgroup AND UM.parent_id !=0 AND UM.show_in_menu = 1 AND UM.is_active = 1 ORDER BY sort_order ASC");
        }
