...
class MY_Controller extends CI_Controller
{
    ...
    public function fetchsidebar_childMenuById($child_id)
    {
        if ($this->session->userdata('user_login')['id_group'] == 1) {
            $query = $this->db->query("SELECT * FROM tbl_user_menu WHERE parent_id = $child_id AND show_in_menu = 1 AND is_active = 1 ORDER BY sort_order ASC");
        } else {
            $sgroup = $this->session->userdata('user_login')['id_sgroup'];
            $query = $this->db->query("SELECT * FROM tbl_user_menu AS UM , tbl_user_permission UP WHERE UM.id_menu = UP.id_menu AND UP.per_select = 1 AND UP.id_sgroup = $sgroup AND UM.parent_id = $child_id AND UM.show_in_menu = 1 AND UM.is_active = 1 ORDER BY sort_order ASC");
        }
